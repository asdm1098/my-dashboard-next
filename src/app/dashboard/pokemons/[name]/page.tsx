...
interface Props {
  params: Promise<{ name: string }>; // Nota el tipo Promise aquí
}
...
// Generar metadata con soporte para promesas en params
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const { name: PokemonName } = await params; // Resolver la promesa aquí
    ...
    return {
      ...
      description:
        "Hubo un error al cargar los datos del Pokémon. Intenta nuevamente más tarde.",
    };
  }
}
...
const getPokemon = async (name: string): Promise<Pokemon> => {
  try {
    const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, {
      next: { revalidate: 60 * 60 * 30 * 6 }, // Configuración de cache
    }).then((resp) => resp.json());
    ...
    notFound(); // Manejo de error en caso de que no se encuentre el Pokémon
  }
};

// Componente principal
export default async function PokemonPage({ params }: Props) {
  const name = (await params).name; // Resolver la promesa
